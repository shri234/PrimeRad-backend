...
const { applySessionAccessControl } = require("../config/sessionAccessHelper");
    ...
    switch (sessionType) {
      ...
      default:
        const [dicomDocs, recordedDocs, liveDocs] = await Promise.all([
          ...
          LivePrograms.find({}).populate(populateFacultyQuery),
        ]);
...
async function getTopRatedCases(req, res) {
  try {
    ...
    const filteredCases = applySessionAccessControl(
      getCases.map((c) => ({ ...c.toObject(), sessionType: "Dicom" })),
      req.userAccess,
      2
    );
...
async function getTopRatedLectures(req, res) {
  try {
    ...
    // Apply access control
    const filteredLectures = applySessionAccessControl(
      getLectures.map((l) => ({ ...l.toObject(), sessionType: "Vimeo" })),
      req.userAccess,
      2
    );
...
async function getRecentItems(req, res) {
  try {
    ...
    const getSessions = [
      ...getDicomCases.map((s) => ({ ...s.toObject(), sessionType: "Dicom" })),
      ...getRecordedLectures.map((s) => ({
        ...s.toObject(),
        sessionType: "Vimeo",
      })),
      ...getLivePrograms.map((s) => ({ ...s.toObject(), sessionType: "Live" })),
    ];
...
async function getSessionsByDifficulty(req, res) {
  try {
    ...
    const populateFacultyQuery = { path: "faculty", select: "name image" };
    const difficultyFilter = { pathologyId: pathologyId };
...
async function getTopWatchedSessions(req, res) {
  try {
    ...
    // Apply access control
    const filteredResult = applySessionAccessControl(result, req.userAccess, 2);
...
async function getWatchedSessions(req, res) {
  try {
    ...
    const liveProgramIds = playbackProgress
      .filter((p) => p.sessionModelType === "LiveProgram")
      .map((p) => p.sessionId);
...
async function getUpcomingLivePrograms(req, res) {
  try {
    ...
    // Apply access control
    const filteredPrograms = applySessionAccessControl(
      upcomingPrograms.map((p) => ({ ...p.toObject(), sessionType: "Live" })),
      req.userAccess,
      2
    );

    return res.status(200).json({
      message: `Got ${filteredPrograms.length} upcoming live programs successfully`,
